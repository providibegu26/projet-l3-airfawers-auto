...
async function testMisesSuccessivesCorrige() {
  try {
    console.log('🧪 Test des mises à jour successives (corrigé)...');
    ...
    for (let i = 0; i < misesAJour.length; i++) {
      ...
      const updateResponse = await fetch(`http://localhost:4000/api/admin/vehicules/${vehicle.immatriculation}/mileage`, {
        ...
        body: JSON.stringify({
          newMileage: miseAJour.kilometrage,
      ...
      if (!updateResponse.ok) {
        ...
        console.error(`❌ Erreur mise à jour ${i + 1}:`, errorData);
        continue;
      }
