  ...
  useEffect(() => {
    if (isOpen) {
      fetch('http://localhost:4000/api/admin/chauffeurs')
        .then(res => res.json())
        .then(data => {
          // Filtrer les chauffeurs déjà assignés à un véhicule
          fetch('http://localhost:4000/api/admin/vehicules')
            .then(res2 => res2.json())
            .then(data2 => {
              const vehicules = data2.vehicules || [];
              const chauffeursAvecVehicule = new Set(vehicules.filter(v => v.chauffeurId).map(v => v.chauffeurId));
              const filtered = (data.chauffeurs || []).filter(c => !chauffeursAvecVehicule.has(c.id) || (vehicle && c.id === vehicle.chauffeurId));
